...
- **Sucesso e ordem das operações (Task 17)**: o `Invoice` retornado (já `Closed`, com `closedAtUtc` preenchido) substitui o signal `invoice`; em seguida `ChangeDetectorRef.detectChanges()` roda uma passada síncrona de detecção de mudanças (`handlePrintSuccess`, `invoice-detail-page.ts`) — necessária porque um `signal.set()` sozinho não garante que o DOM já foi atualizado no mesmo tick, e `app-invoice-print-view` é um nó irmão dentro do mesmo template de `InvoiceDetailPage`, então essa única chamada já sincroniza os dois. Só então `window.print()` é chamado, garantindo que a nota impressa sempre mostre `Status: Fechada` (nunca o estado anterior à baixa). O toast de sucesso **não** é mais disparado antes de `window.print()`: um listener do evento padrão `afterprint` (disparado pelo navegador quando a caixa de diálogo de impressão é fechada, seja imprimindo ou cancelando) é quem dispara a notificação — não um `setTimeout` arbitrário, já que `window.print()` bloqueia a execução do script em alguns navegadores (Chrome) mas não em outros (Firefox), tornando "logo depois da chamada" um sinal não confiável entre navegadores.
- **Erros amigáveis**: `handlePrintError` mapeia `HttpErrorResponse.status` para mensagens em português exibidas em `<p role="alert" class="invoice-detail-page__print-error">`: 404 ("Nota não encontrada."), 409 (usa `error.error?.detail` do `ProblemDetails` quando presente — cobre tanto "já fechada" quanto "saldo insuficiente" sem precisar distinguir os dois casos no frontend — com fallback genérico), 0/503 ("Serviço de estoque indisponível..."), e uma mensagem genérica para qualquer outro status. A chamada usa `SKIP_ERROR_NOTIFICATION` (mesmo padrão dos demais métodos de `InvoicesService`) para não duplicar o toast genérico do interceptor por cima dessa UI dedicada. Em caso de erro, `window.print()` nunca é chamado e o toast de sucesso nunca é disparado.
- **Componente imprimível (`InvoicePrintView`, `invoice-print-view.ts/.html/.scss`)**: `standalone`, recebe a nota via `@Input({ required: true })`, renderizado como irmão do `<section>` principal em `invoice-detail-page.html` (fora do bloco marcado `invoice-detail-page__no-print`). Fica com `:host { display: none; }` na tela normal e só aparece dentro de `@media print { :host { display: block; } }`, mostrando número, status, datas e a tabela de itens em HTML/CSS simples (sem Angular Material) para impressão limpa. `invoice-detail-page.scss` complementa isso escondendo o conteúdo interativo (`.invoice-detail-page__no-print`) dentro de `@media print`, e uma regra global central em `src/styles.scss` esconde, dentro do mesmo bloco `@media print`: a navegação/toolbar do shell (`.app-shell__topbar`) e todo overlay do Angular CDK (`.cdk-overlay-container`, `.cdk-overlay-backdrop` — cobre snackbars, diálogos, menus e tooltips com uma única regra central, sem `::ng-deep` e sem regras espalhadas por componente, já que o CDK anexa esses overlays diretamente sob `<body>`, fora da árvore de qualquer componente). Isso garante que `window.print()` produza apenas o conteúdo da nota — nunca o toast de sucesso, que antes da Task 17 podia aparecer dentro da impressão por não haver nenhuma regra escondendo `.cdk-overlay-container` no `@media print`.
- **Cabeçalhos/rodapés do navegador (fora do controle da aplicação)**: data/hora, título da página, URL e número de página são adicionados pelo próprio navegador na janela de impressão, não pelo Angular — nenhuma técnica do lado da aplicação os remove de forma confiável entre navegadores. Para uma impressão sem esses elementos, o usuário deve desmarcar "Cabeçalhos e rodapés" em "Mais configurações" na janela de impressão do navegador.
- **Testes**: `invoice-detail-page.spec.ts` cobre botão habilitado/desabilitado por status, spinner + botão desabilitado durante a chamada, chamada única do serviço mesmo com múltiplos cliques em sequência antes do próximo `detectChanges()`, sucesso (atualização do badge, `window.print()` chamado exatamente uma vez), confirmação de que o DOM de `app-invoice-print-view` já mostra `Fechada` no momento exato em que `window.print()` é invocado (inspecionado de dentro do próprio mock de `window.print`), toast de sucesso não disparado antes de `window.print()` e disparado exatamente uma vez após o evento `afterprint` (mesmo se disparado mais de uma vez), mensagens para 404/409/503 na impressão (com `notification.success` explicitamente não chamado), e reabilitação do botão após falha. `invoice-print-view.spec.ts` cobre a renderização isolada do componente (número, status, datas, itens). `invoices.service.spec.ts` cobre `print(id)` (`POST` correto, resposta tipada). `styles.print.spec.ts` compila `src/styles.scss` de verdade (via `styleUrl` em um componente de teste com `ViewEncapsulation.None`, sem duplicar CSS) e confirma, por inspeção do `CSSOM` (`document.styleSheets`), que o bloco `@media print` contém as regras de `.cdk-overlay-container`/`.cdk-overlay-backdrop` e `.app-shell__topbar`, e que nenhuma regra esconde `app-invoice-print-view`.
